...
<b>особенности работы:</b><br>
    ...
    -   хром вроде как блокирует автозаполнения пароля. в фф не работает на инпутах tel, password, возможно, каких-то еще (на инпут накладывается желтый фильтр и не работают ховеры). Можно подключить проверку на браузер, и, если зашли с фф, то выставлять для них задержку transition-delay = 0, тогда ховеры работают, а для удаления желтого фильтра выставить background:inherit и background-color: #fff (здесь подставить вместо #fff цвет инпута в "спокойном" состоянии), либо  при автозаполнении некоторых инпутов (телефон, пароль, возможно, какие-то еще) background будет с наложенным желтым фильтром <br>
