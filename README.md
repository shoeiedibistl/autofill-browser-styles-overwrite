...
<b>особенности работы:</b><br>
    ...
    -   хром вроде как блокирует автозаполнения пароля. в фф не работает на инпутах tel, password, возможно, каких-то еще (инпут остается в стилях для режима focus и не работают ховеры). Можно подключить проверку на браузер, и, если зашли с фф, то выставлять задержку transition-delay = 0, тогда ховеры работают, но при автозаполнении background будет как в состоянии :focus. фф, похоже, по умолчанию не перебивает инпут своими стилями, а берет некоторые из фокуса  <br>
