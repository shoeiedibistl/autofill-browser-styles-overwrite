...
<b>особенности работы:</b><br>
    ...
    -   хром вроде как блокирует автозаполнения пароля. в фф не работает на инпутах tel, password, возможно, каких-то еще (на инпут накладывается желтый фильтр и не работают ховеры). Можно подключить проверку на браузер, и, если зашли с фф, то выставлять задержку transition-delay = 0, тогда ховеры работают, но при автозаполнении ряда инпутов (телефон, пароль, возможно, какие-то еще) background будет с наложенным желтым фильтром <br>
